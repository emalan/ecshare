...
/**
 * Base class for Application Pages that supplies Content and other
 * functionality.
 * <p>
 * The Content Admin will supply and allow editing of meta information for this
 * page. So make sure you do not duplicate the following (title, author,
 * keywords, description).
 * </p>
 * <p>
 * Layout functionality is supplied using YUI, so make sure to leverage that for
 * your page layout and in your style sheets. Popup login functionality and a
 * Language switcher can be activated by overiding the relevant methods.
 * </p>
 * 
 * @author Eugene Malan
 * 
...
public abstract class CmsPage extends WebPage {
    ...
    private void commonInit() {
        ...
        // store last page so we can return from Admin Pages to last used site
        // page
    ...
    private void setupPopupLogin() {
        ...
        final Animator animator = new Animator(700).addSubject(
                AnimatorSubject.numeric("loginPopup", "opacity", 0.0, 1.0)).addSubject(
                AnimatorSubject.discrete("loginPopup", "display", "none", "", 0.1));
    ...
    /**
     * Crossfade support allows you to identify a Rotating list by adding a
     * class of .crossfade
     * 
    ...
    /**
     * Add a Text Content Panel to this page. NOTE: be careful when using this
     * method from an abstract class as new Panels will be created for each
     * implementation.
     * 
     * @param id
     */
    protected Panel addContentPanel(final String id) {
        Panel panel = getContentPanel(id, getPageName());
        add(panel);
        return panel;
    }

    protected Panel addAppContentPanel(final String id) {
        Panel panel = getContentPanel(id, APPNODE);
        add(panel);
        return panel;
    }
    
    private Panel getContentPanel(final String id, final String node) {
        final PageParameters params = new PageParameters();
        params.set(0, id);
        params.set(1, node);

        return new ContentPanel(id, getContentModel(id, node), params);
    }
    
    private IModel<String> getContentModel(final String id, final String node) {
        final PageData page = getRepositoryService().getPage(node);
        final ContentData contentData = getRepositoryService().getContent(page, id);
        return new AbstractReadOnlyModel<String>() {
            ...
            public String getObject() {
                return getRepositoryService().getContentText(contentData, getSession().getLocale());
            }
        };
    }
     ...
     * @param id
     * @param subject
     *            - this will appear as subject in the email
     * @return
     */
    ...
    /**
     * Add exhibit panel to page. This method shows the new preferred way to
     * construct components so that the components do not need to know where the
     * data comes from.
     * 
     * @return Panel
     * @param id
    ...
    protected Panel addExhibitPanel(String id) {
        ...
        Panel exhibit = new ExhibitPanel(id);
        exhibit.add(getContentPanel(ExhibitPanel.CONTENT + "1", ExhibitPanel.NODE));

        final Panel albumPanel = new AlbumPanel("nested", album, imagesModel);
        IModel<String> contentModel = getContentModel(ExhibitPanel.CONTENT + "2", ExhibitPanel.NODE);
        final PageParameters params = new PageParameters();
        params.set(0, ExhibitPanel.CONTENT + "2");
        params.set(1, ExhibitPanel.NODE);
        exhibit.add(new ContentPanel(ExhibitPanel.CONTENT + "2", contentModel, params, albumPanel));
        ...
        for (final Class<? extends Page> page : pages) {
            ...
            items.add(new IMenuItem() {
                ...
                public String getItemName() {
                    PageMetaLangData pageInfo = getRepositoryService().getPageMetaLang(getSession().getLocale(),
                            pageData);
